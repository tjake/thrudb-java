...
public class RealTimeLuceneIndex implements LuceneIndex, Runnable {
	...
	CountDownLatch shutdownSyncLatch = new CountDownLatch(1); // counted down on
																// final sync()
	...
	RealTimeLuceneIndex(String indexRoot, String indexName) throws IOException,
		...
		if (createIndex) {
			diskWriter = new IndexWriter(indexLocation, analyzer, createIndex,
			...
			diskWriter = new IndexWriter(diskDirectory, analyzer,
					IndexWriter.MaxFieldLength.UNLIMITED);
		}
	...
	public synchronized void put(String key, Document document,
			Analyzer analyzer) throws ThrudexException {
	...
	public SearchResponse search(SearchQuery query, Analyzer analyzer)
			throws ThrudexException {
		...
		try {
			...
			synchronized (this) {
				...
				multiSearcher = new ParallelMultiSearcher(searchersList
						.toArray(searchers));
			...
			TopDocs result = null;
			try {
				result = multiSearcher.search(parsedQuery, myFilter,
						query.offset + query.limit, sortBy);
			} catch (Exception e) {
			...
			if (query.isPayload()) {
				fieldSelector = new MapFieldSelector(new String[] {
						DOCUMENT_KEY, PAYLOAD_KEY });
			} else {
				fieldSelector = new MapFieldSelector(
						new String[] { DOCUMENT_KEY });
			}

			for (int i = query.offset; i < result.totalHits
					&& i < (query.offset + query.limit); i++) {

				Element el = new Element();
				el.setIndex(query.index);

				Document d = multiSearcher.doc(result.scoreDocs[i].doc,
						fieldSelector);
	...
	public void run() {
		while (true) {
			try {
				...
				//early escape for shutdown-hook
				if(shutdownLatch.getCount() == 0 && ramWriter.numDocs() == 0){
				...
				synchronized (this) {
					logger.debug("deleted " + deletedDocuments.size()
							+ " documents");
				...
				logger.debug("Writing " + prevRamReader.numDocs()
						+ " docs to disk");
				// now merge the indexes

				diskWriter
						.addIndexesNoOptimize(new Directory[] { prevRamDirectory });
				...
				if (shutdownLatch.getCount() > 0) {
					...
					try {
						TopDocs result = primeSearcher.search(
								new MatchAllDocsQuery(), 100);
						logger.debug("primed index: " + result.totalHits);
					} catch (Exception e) {
						logger.error("error priming searcher"
					...
					synchronized (this) {
						...
						logger.debug("Have " + diskReader.numDocs()
								+ " docs on disk");
