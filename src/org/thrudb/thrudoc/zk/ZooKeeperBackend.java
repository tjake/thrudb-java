...
public final class ZooKeeperBackend implements Iface, Runnable, Watcher {
	...
	public ZooKeeperBackend(String zkAddress, int port,
			Thrudoc.Iface delegateHandler) {
		this.zkAddress = zkAddress;
		this.delegateHandler = delegateHandler;
		this.thrudocPort = port;

		try {
			instanceId = new String(delegateHandler.get("thrudb",
					ThrudocBackend.SPECIAL_KEY));
			...
			throw new RuntimeException(
					"Problem identifying this thrudb instance on zookeeper... bailing");
		}

		this.serverList = new ArrayList<String>();
		this.serverInfo = new ConcurrentHashMap<String, ServerInstance>();
		this.bucketList = new ArrayList<String>();
		this.bucketInfo = new ConcurrentHashMap<String, List<BucketInstance>>();
	...
	private void createServerList() throws KeeperException,
		...
		try {
			...
			zkServer.create(serverListNode + "/id", obj, Ids.OPEN_ACL_UNSAFE,
	...
	private void createBucketList() throws KeeperException,
			InterruptedException, TException, ThrudocException {

		// create the bucketlistNode
		try {
			zkServer.create(bucketListNode, null, Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
		} catch (KeeperException.NodeExistsException e) {
			// this is fine.
		}
		...
		for (String bucket : buckets) {
			...
			try {
				zkServer.create(bucketListNode+"/"+bucket, null, Ids.OPEN_ACL_UNSAFE,
						CreateMode.PERSISTENT);
			} catch (KeeperException.NodeExistsException e) {
				// this is fine.
			}
			...
			try {
				...
				// add myself to the instance list
				zkServer.create(bucketListNode + "/"+ bucket +"/id", obj, Ids.OPEN_ACL_UNSAFE,
						CreateMode.EPHEMERAL_SEQUENTIAL);
	 ...
	 * @return
	 */
	public ServerInstance getInstanceForBucket(String bucket, String key, boolean write) 
